...
<body class="bg-white">
    <div class="container pt-5 mx-auto">
        <h1 class="text-3xl font-bold mb-5">DATA PERSURATAN</h1>
        @if(session('success'))
        ...
        <table class="table-auto w-full">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="border px-4 py-2">{{$item->jenis_persuratan}}</td>
                    <td class="border px-4 py-2">{{$item->keterangan_persuratan}}</td>
                    <td class="border px-4 py-2">{{$item->tanggal_persuratan}}</td>
                    <td class="border px-4 py-2">{{$item->status_persuratan}}</td>
                    <td class="border px-4 py-2">
                        <div class="flex gap-3">
                            <a href="/persuratan/{{$item->id_persuratan}}/edit"
                                class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">Edit</a>
                            <a href="/persuratan/{{$item->id_persuratan}}"
                                class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Detail</a>
                            <form action="{{url('persuratan/'.$item->id_persuratan)}}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
