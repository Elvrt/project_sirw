        ...
        <div class="bg-gradient-to-br from-hijau2 to-cream-muda rounded-lg pb-20">
            @foreach ($strukturRw as $data)
                <div class="ml-20 mr-10 mt-5 md-5">
                    <p class="text-center font-bold text-2xl text-army-gelap">{{ $data->nama_struktur }}</p>
                    <div class="grid grid-cols-3 mt-5 ">
                        ...
                        <div class="bg-gradient-to-r from-abu-putih to-abu-putih-muda opacity-50 rounded-lg col-span-2">
                            <p class="text-left ml-2 text- text-army-gelap">Nama‎ ‎ ‎ ‎ : {{ $data->warga->nama_warga }}</p>
                            <p class="text-left ml-2 text- text-army-gelap">Jabatan‎ : {{ $data->nama_struktur }}</p>
                            <p class="text-left ml-2 text- text-army-gelap">Alamat ‎ : {{ $data->warga->alamat }}</p>
                            <p class="text-left ml-2 text- text-army-gelap">No.Telp : {{ $data->warga->nomor_telepon }}</p>
                        </div>
                    </div>
                </div>
            @endforeach
            ...
            <div class="grid grid-cols-6 gap-4 mr-10 mb-5 mt-5">
                @foreach ($ketuaRt as $data)
                    <div class="ml-10 text-center">
                        <p class="font-bold text-xl text-army-gelap">{{ $data->nama_struktur }}</p>
                        <img src="{{ url('assets/img/profile.png') }}" alt="profile" width="100">
                    </div>
