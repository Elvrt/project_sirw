...
<body class="bg-white">
    <div class="container pt-5 mx-auto">
        <h1 class="text-3xl font-bold mb-5">DATA AGENDA</h1>
        @if(session('success'))
        ...
        <table class="table-auto w-full">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="border px-4 py-2">{{$item->gambar_agenda}}</td>
                    <td class="border px-4 py-2">{{$item->judul_agenda}}</td>
                    <td class="border px-4 py-2">{{$item->deskripsi_agenda}}</td>
                    <td class="border px-4 py-2">{{$item->tanggal_agenda}}</td>
                    <td class="border px-4 py-2">
                        <div class="flex gap-3">
                            <a href="/agenda/{{$item->id_agenda}}/edit"
                                class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">Edit</a>
                            <a href="/agenda/{{$item->id_agenda}}"
                                class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Detail</a>
                            <form action="{{url('agenda/'.$item->id_agenda)}}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
