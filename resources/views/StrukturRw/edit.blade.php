...
<body class="bg-white">
    <div class="container pt-5 mx-auto">
        <h1 class="text-3xl font-bold mb-5">EDIT DATA STRUKTUR RW</h1>
        <form action="{{url('/struktur-rw/'.$data->id_struktur)}}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-4">
                <label for="kode_struktur" class="block text-sm font-bold mb-2">Kode</label>
                <input type="text" name="kode_struktur" value="{{$data->kode_struktur}}" id="kode_struktur" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
            </div>
            <div class="mb-4">
                <label for="nama_struktur" class="block text-sm font-bold mb-2">Nama Struktur</label>
                <input type="text" name="nama_struktur" value="{{$data->nama_struktur}}" id="nama_struktur" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
            </div>
            <div class="mb-4">
